...
void applyExternalForce(Context* context, float dt)
{
  for (int i = 0; i<context->num_particles; i++){
    context->particles[i].velocity.y -= (dt*15.0F); // gravité 
    //context->particles[i].velocity.y += dt*exp(pow(context->particles[i].temperature-1, 5))*15.0F; //force de convexion exponentielle
    //context->particles[i].velocity.y += (dt*(pow(context->particles[i].temperature, 2))*20.0F)/context->particles[i].inv_mass; //quadratique
    context->particles[i].velocity.y += (dt*context->particles[i].temperature*20.0F);
    ...
    for (int j = 0; j<context->num_particles; j++){
          if (i != j){
            ...
            if (distance < r+R){
                ...
                float delta = exp(pow(context->particles[i].temperature, 10))/(100);

                context->particles[i].next_pos.x += (0.5-delta)*dx; //ici le coeficient représente la viscosité su fluide, plus il inférieur a 0.5 plus les colision se propage, devient vite instable
                context->particles[i].next_pos.y += (0.5-delta)*dy; //la limite d'instabilité est est de est 0.485

                context->particles[j].next_pos.x -= (0.5+delta)*dx;
                context->particles[j].next_pos.y -= (0.5+delta)*dy;
                ...
                context->particles[i].temperature = 0.9*temperature_i+0.1*temperature_j;
                context->particles[j].temperature = 0.9*temperature_j+0.1*temperature_i;
            }
          }
    }
  }
...
void checkContactWithPlane(Context* context, int particle_id){
    ...
    for (int i = 0; i<context->num_ground_plane; i++){
     ...
     if (context->particles[particle_id].next_pos.y < -(14.0-context->particles[particle_id].radius)){ // sol
      
      
      
      context->particles[particle_id].next_pos.y = -(14.0-context->particles[particle_id].radius)+0.001;
     ...
     if (context->particles[particle_id].next_pos.y > (14.0-context->particles[particle_id].radius)){ //haut
        context->particles[particle_id].next_pos.y = (14.0-context->particles[particle_id].radius)-0.001;
